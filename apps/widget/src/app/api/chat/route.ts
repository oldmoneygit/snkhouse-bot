...
export async function POST(request: NextRequest) {
  try {
    ...
    if (existingCustomer) {
      ...
      console.log(
        "📝 [Widget API] Cliente criado no Supabase:",
        customerRecord.id,
      );
    }
    ...
    if (emailMatch) {
      ...
      console.log(
        "📧 [Widget API] Usando email do onboarding:",
        sanitizeEmail(effectiveEmail),
      );
    }
    ...
    if (wooCustomerId) {
      console.log(
        "♻️ [Widget API] Reutilizando woocommerce_id do cache:",
        wooCustomerId,
      );
    }
    ...
    if (!wooCustomerId || emailWasUpdated) {
      console.log(
        "🔍 [Widget API] Buscando woocommerce_id no WooCommerce para:",
        sanitizeEmail(effectiveEmail),
      );
      ...
      try {
        ...
        if (wooCustomer) {
          ...
          if (updateError) {
            console.error(
              "❌ [Widget API] Erro ao salvar woocommerce_id no Supabase:",
              updateError,
            );
          } else {
            if (emailWasUpdated) {
              console.log(
                "✅ [Widget API] WooCommerce customer_id atualizado após mudança de email:",
                wooCustomerId,
              );
            } else {
              console.log(
                "✅ [Widget API] WooCommerce customer_id encontrado e salvo:",
                wooCustomerId,
              );
            }
          }
          ...
          console.log(
            "⚠️ [Widget API] Cliente não encontrado no WooCommerce para email:",
            sanitizeEmail(effectiveEmail),
          );
        }
      } catch (integrationError) {
        console.error(
          "❌ [Widget API] Erro ao consultar WooCommerce:",
          integrationError,
        );
      }
    }
    ...
    if (activeConversationId) {
      ...
      if (savedEmail !== effectiveEmail) {
        ...
        console.log(
          "🔄 [Widget API] Email da conversa atualizado de",
      ...
      console.log(
        "💬 [Widget API] Nova conversa criada com email:",
        sanitizeEmail(effectiveEmail),
      );
    }

    // Garantir que activeConversationId nunca seja null
    if (!activeConversationId) {
      throw new Error("No fue posible obtener o crear la conversación");
    }

    // 4) Construir hist�rico para a IA
    const { data: conversationMessages, error: conversationMessagesError } =
      await supabaseAdmin
        .from("messages")
        .select("role, content")
    ...
    if (userMessageError) {
      console.error(
        "? [Widget API] Falha ao salvar mensagem do usu�rio:",
        userMessageError,
      );
    }
    ...
    await trackAIResponse({
      ...
      total_tokens: Math.ceil(
        (lastUserMessage.length + response.content.length) / 4,
      ),
    ...
    if (assistantMessageError) {
      console.error(
        "? [Widget API] Falha ao salvar resposta do assistente:",
    ...
    try {
      ...
      console.error(
        "? [Widget API] Error tracking failed response:",
        trackError,
      );
    }
