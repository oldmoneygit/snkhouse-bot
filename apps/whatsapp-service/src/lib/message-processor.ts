...
import { findOrCreateCustomer } from "./customer-helper";
import {
...
export async function processIncomingWhatsAppMessage(
  ...
  try {
    ...
    console.log(
      "🔍   Querying messages WHERE conversation_id =",
      conversation.id,
    );
    ...
    // =====================================================
    console.log(
      "[MessageProcessor] 📤 PRIORITY: Sending WhatsApp message FIRST...",
    );
    ...
    console.log(
      "[MessageProcessor] 💬 Message length:",
      response.content.length,
    );
    console.log(
      "[MessageProcessor] 💬 Message preview:",
    ...
    try {
      ...
      console.log(
        "[MessageProcessor] ✅✅✅ MESSAGE SENT SUCCESSFULLY! ✅✅✅",
      );
      ...
      console.error("[MessageProcessor] ❌ ERROR SENDING MESSAGE:", {
        ...
        stack:
          sendError instanceof Error
            ? sendError.stack?.substring(0, 300)
            : undefined,
      });
    ...
    try {
      ...
      console.log(
        "✅ DEBUG #8 - Current message count in conversation:",
        verifyHistory.length,
      );
    } catch (saveError: any) {
      console.error(
        "❌ DEBUG #8 - Failed to save assistant message:",
        saveError.message,
      );
    ...
    try {
      ...
      console.warn(
        "[MessageProcessor] ⚠️ Failed to mark as read (ignored):",
        markError.message,
      );
    ...
    console.log(
      "[MessageProcessor] 🎉🎉🎉 Processing completed successfully! 🎉🎉🎉",
    );
    ...
    console.log(
      "[MessageProcessor]   - Total messages in conversation:",
      aiMessages.length + 1,
    );
  } catch (error: any) {
    console.error("[MessageProcessor] ❌ ERROR:", error);
    console.error(
      "[MessageProcessor] ❌ Error stack:",
    ...
    try {
      console.log(
        "[MessageProcessor] 🚨 Attempting to send error message to user...",
      );

      await whatsappClient.sendMessage({
        to: message.from,
        message:
          "Disculpá, tuve un problema técnico. Intentá de nuevo en unos minutos. 🙏",
      });

      console.log("[MessageProcessor] ✅ Error message sent to user");
    } catch (sendError: any) {
      console.error(
        "[MessageProcessor] ❌ Failed to send error message:",
        sendError,
      );
