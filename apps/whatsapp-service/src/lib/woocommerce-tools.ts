...
export const woocommerceTools = {
  ...
  getOrderDetails: {
    ...
    execute: async ({ order_id, email }: { order_id: string; email: string }) => {
      console.log(`[WooCommerce Tool] getOrderDetails: order=${order_id}, email=${email.substring(0, 5)}***`);
      ...
      try {
        ...
        console.log(`[WooCommerce Tool] ✅ Order found: #${order.number}, status: ${order.status}`);
        ...
        console.error('[WooCommerce Tool] ❌ getOrderDetails error:', error.message);
        return {
          found: false,
  ...
  checkProductStock: {
    ...
    execute: async ({ product_id, size }: { product_id: string; size?: string }) => {
      console.log(`[WooCommerce Tool] checkProductStock: product_id=${product_id}, size=${size || 'N/A'}`);
      ...
      try {
        ...
        if (size) {
          console.log(`[WooCommerce Tool] ⚠️ Size-specific stock not implemented yet, returning general stock`);
        }

        console.log(`[WooCommerce Tool] ✅ Stock check: ${stockInfo.in_stock ? 'Available' : 'Out of stock'}`);
        ...
        console.error('[WooCommerce Tool] ❌ checkProductStock error:', error.message);
        return {
