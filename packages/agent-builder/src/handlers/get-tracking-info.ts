...
export async function getTrackingInfoHandler(input: GetTrackingInfoInput) {
  try {
    console.log(
      `📦 [GetTracking] Fetching tracking for order: ${input.order_id}`,
    );
    ...
    const providerMeta = order.meta_data?.find(
      (meta: any) =>
    ...
    const result = {
      ...
      tracking_url: trackingCode
        ? generateTrackingUrl(provider, trackingCode)
        : null,
    ...
    console.log(
      `✅ [GetTracking] Tracking info retrieved. Has tracking: ${hasTracking}`,
    );
