...
export async function getOrderDetailsHandler(input: GetOrderDetailsInput) {
  try {
    ...
    console.log(`📋 [GetOrderDetails] Order found, customer: ${order.billing.email}`);

    // SEGURANÇA: Validar email do cliente
    if (order.billing.email.toLowerCase() !== input.customer_email.toLowerCase()) {
      console.warn(`⚠️ [GetOrderDetails] Email mismatch! Order: ${order.billing.email}, Provided: ${input.customer_email}`);
      return {
        success: false,
        error: 'El email no coincide con el pedido. Por favor verifica tus datos.'
      };
    }

    // Extraer código de tracking de meta_data
    const tracking = order.meta_data?.find((meta: any) =>
    ...
    const result = {
      ...
      products: order.line_items.map((item: any) => ({
        ...
        size: item.meta_data?.find((m: any) =>
    ...
    return {
      success: false,
      error: 'Hubo un problema al consultar el pedido. Intentá de nuevo en unos segundos.'
    };
  }
}
