...
class WooCommerceDiagnostic {
  ...
  constructor() {
    ...
    console.log(`🔐 Consumer Secret: ${this.consumerSecret.substring(0, 15)}...`);
    console.log('━'.repeat(80));
  }

  private addResult(test: string, success: boolean, status?: number, message: string, details?: any, suggestion?: string) {
  ...
  async testBasicConnectivity(): Promise<boolean> {
    ...
    try {
      ...
      this.addResult('Site Online', true, response.status, 'Site respondendo normalmente');
    } catch (error: any) {
      this.addResult('Site Online', false, error.response?.status, 
        'Site não está respondendo ou timeout', error.message,
    ...
    try {
      ...
      this.addResult('WordPress REST API', true, response.status, 'WordPress REST API ativa');
    } catch (error: any) {
      this.addResult('WordPress REST API', false, error.response?.status,
        'WordPress REST API não está ativa ou acessível', error.message,
  ...
  async testWooCommerceAPI(): Promise<boolean> {
    ...
    try {
      ...
      this.addResult('WooCommerce API Endpoint', true, response.status, 'API endpoint acessível');
      
      if (response.data?.namespace) {
        console.log(`   📋 Namespace: ${response.data.namespace}`);
        console.log(`   📋 Routes disponíveis: ${Object.keys(response.data.routes || {}).length}`);
      }
    } catch (error: any) {
      const status = error.response?.status;
      if (status === 404) {
        this.addResult('WooCommerce API Endpoint', false, status,
          'WooCommerce não está instalado ou API não está ativa', error.response?.data,
          'Instale o plugin WooCommerce e ative a API REST');
      } else {
        this.addResult('WooCommerce API Endpoint', false, status,
          'Erro ao acessar API WooCommerce', error.message,
  ...
  async testAuthenticationMethods(): Promise<boolean> {
    ...
    try {
      ...
      this.addResult('Basic Auth', true, response.status, 'Autenticação básica funcionando');
      anyMethodWorked = true;
    } catch (error: any) {
      const status = error.response?.status;
      if (status === 401) {
        this.addResult('Basic Auth', false, status,
          'Credenciais inválidas ou sem permissão', error.response?.data,
          'Verifique se as credenciais estão corretas e têm permissão de leitura');
      } else {
        this.addResult('Basic Auth', false, status,
          'Erro na autenticação básica', error.response?.data);
      }
    }
    ...
    try {
      ...
      this.addResult('Query Parameters', true, response.status, 'Autenticação por query params funcionando');
      anyMethodWorked = true;
    } catch (error: any) {
      const status = error.response?.status;
      if (status === 401) {
        this.addResult('Query Parameters', false, status,
          'Credenciais inválidas mesmo com query params', error.response?.data,
          'As credenciais podem estar incorretas ou expiradas');
      } else {
        this.addResult('Query Parameters', false, status,
          'Erro na autenticação por query params', error.response?.data);
      }
    }
    ...
    try {
      ...
      this.addResult('Headers Customizados', true, response.status, 'Autenticação por headers funcionando');
      anyMethodWorked = true;
    } catch (error: any) {
      this.addResult('Headers Customizados', false, error.response?.status,
        'Autenticação por headers não suportada', error.response?.data);
    }
  ...
  async testAPIEndpoints(): Promise<boolean> {
    ...
    const endpoints = [
      ...
      { name: 'Categories (Lista)', url: '/products/categories?per_page=1', method: 'GET' },
    ];
    ...
    for (const endpoint of endpoints) {
      try {
        const response = await axios.get(`${this.url}/wp-json/wc/v3${endpoint.url}`, {
          auth: {
            username: this.consumerKey,
            password: this.consumerSecret,
          },
        ...
        this.addResult(`Endpoint: ${endpoint.name}`, true, response.status, 'Endpoint acessível');
        workingEndpoints++;
        ...
        if (endpoint.name === 'Categories (Lista)' && response.data.length > 0) {
        ...
        this.addResult(`Endpoint: ${endpoint.name}`, false, status,
          'Endpoint não acessível', error.response?.data, suggestion);
      }
    }
  ...
  async testCredentialsValidation(): Promise<boolean> {
    ...
    this.addResult('Formato Consumer Key', keyValid, undefined,
      keyValid ? 'Formato correto' : 'Formato incorreto',
      { key: this.consumerKey.substring(0, 10) + '...' },
      keyValid ? undefined : 'Consumer Key deve começar com "ck_" e ter 43 caracteres');

    this.addResult('Formato Consumer Secret', secretValid, undefined,
      secretValid ? 'Formato correto' : 'Formato incorreto',
      { secret: this.consumerSecret.substring(0, 10) + '...' },
      secretValid ? undefined : 'Consumer Secret deve começar com "cs_" e ter 43 caracteres');
  ...
  async testServerConfiguration(): Promise<void> {
    ...
    try {
      ...
      this.addResult('HTTPS', isHttps, undefined,
      ...
      try {
        ...
        this.addResult('CORS Headers', true, response.status, 'CORS configurado');
      } catch (error: any) {
        this.addResult('CORS Headers', false, undefined, 'CORS não configurado ou bloqueado');
      }
  ...
  generateReport(): void {
    ...
    if (failedTests > 0) {
      ...
      failedResults.forEach((result, index) => {
        console.log(`${index + 1}. ${result.test}: ${result.suggestion || 'Investigar problema'}`);
      });

      console.log('\n💡 PRÓXIMOS PASSOS:');
      console.log('─'.repeat(50));
      
      const authFailed = failedResults.some(r => r.test.includes('Auth') || r.test.includes('Credential'));
      const apiFailed = failedResults.some(r => r.test.includes('API') || r.test.includes('Endpoint'));
      const connectivityFailed = failedResults.some(r => r.test.includes('Online') || r.test.includes('WordPress'));
      ...
      if (apiFailed) {
        ...
        console.log('   - Habilitar API REST em WooCommerce > Configurações > Avançado');
      }

      if (authFailed) {
        console.log('3. 🔑 Recriar credenciais WooCommerce');
        console.log('   - Acessar: WooCommerce > Configurações > Avançado > API REST');
