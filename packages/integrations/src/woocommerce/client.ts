...
export class WooCommerceClient {
  ...
  async getProducts(
    ...
    try {
      ...
      console.log(
        `✅ [WooCommerce] ${response.data.length} produtos encontrados`,
      );
  ...
  async getOrder(
    ...
    try {
      const response = await this.client.get<WooCommerceOrder>(
        `/orders/${orderId}`,
      );

      console.log(
        `✅ [WooCommerce] Pedido encontrado: #${response.data.number}`,
      );
  ...
  async getOrders(params: OrderSearchParams = {}): Promise<WooCommerceOrder[]> {
    ...
    try {
      ...
      console.log(
        `✅ [WooCommerce] ${response.data.length} pedidos encontrados`,
      );
      return response.data;
    } catch (error: any) {
  ...
  async getOrdersByCustomerEmail(email: string): Promise<WooCommerceOrder[]> {
    console.log(
      `🔍 [WooCommerce] Buscando pedidos do cliente: ${sanitizeEmail(email)}`,
    );
    ...
    console.log(
      `✅ [WooCommerce] ${customerOrders.length} pedidos encontrados para ${sanitizeEmail(email)}`,
    );
  ...
  async getCategories(useCache = true): Promise<WooCommerceCategory[]> {
    ...
    try {
      const response = await this.client.get<WooCommerceCategory[]>(
        "/products/categories",
        {
          params: { per_page: 100 },
        },
      ...
      console.log(
        `✅ [WooCommerce] ${response.data.length} categorias encontradas`,
      );
      ...
      console.error(
        `❌ [WooCommerce] Erro ao listar categorias:`,
        error.message,
      );
      throw error;
    }
  ...
  async findCustomerByEmail(
    ...
    console.log(
      `[WooCommerce] Buscando cliente por email: ${sanitizeEmail(email)}`,
    );
    ...
    try {
      ...
      if (customer) {
        ...
        console.log(
          "[WooCommerce] Nenhum cliente encontrado para o email informado",
        );
      }
      return customer;
    } catch (error: any) {
      console.error(
        "[WooCommerce] Erro ao buscar cliente por email:",
        error.message,
      );
      throw error;
    }
